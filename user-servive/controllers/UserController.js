...
const registerFromAuth = async (req, res) => {
  try {
    ...
    if (!_id || !fullName) {
      return res.status(400).json({ success: false, message: 'Missing required fields (_id, fullName)' });
    }

    // Kiểm tra user đã tồn tại chưa
    const existed = await UserModel.findById(_id);
    if (existed) {
      return res.status(200).json({ success: true, message: 'User already exists', user: existed });
    }
